...
OUTPUT_DIR = "output_kmeans_seg_04"
...
def hsv_shift_clipart_segmented(
    ...
    hue_range=(-179, 179),            # in degrees
    sat_range=(-1, 1),          # delta: -1.0 to 1.0
    val_range=(-1, 1),          # delta: -1.0 to 1.0
        ...
        if mutation:
            ...
            if random.random() < 0.7:
                 ...
                 for mut_type in chosen_mutations:
                    if mut_type == "ROTATE":
                        # angle = random.choice([45, -45]) # More angle options
                        ...
                        scale_factor = random.uniform(0.7, 1.3) # Wider resize range
                        ...
                        shift = 40 # Increased shift range
...
def generate_pdf_pair(serial=None, output_dir=OUTPUT_DIR):
    """
    Generates a pair of PDFs (original and mutated) with unique serial numbers.
    Returns the paths to the generated PDFs, or (None, None) on failure.
    """
    ...
    if serial is None:
        serial = max(serial_numbers, default=0) + 1
    ...
    if not clipart_files:
        logger.critical("No clipart files found in the 'clipart' folder. Please add some PNG images. Skipping.")
        return None, None
    ...
    if not original_variants:
        logger.critical("Failed to create initial clipart variants. Skipping.")
        return None, None
    ...
    return orig_pdf, mut_pdf

def generate_bulk_pdfs(n=100, output_dir=OUTPUT_DIR):
    for i in range(n):
        serial = i + 1  # Serial numbers from 1 to n
        logger.info(f"\n=== Generating pair {serial}/{n} ===")
        try:
            orig, mut = generate_pdf_pair(serial=serial, output_dir=output_dir)
            if orig is None or mut is None:
                logger.warning(f"Skipping serial {serial} due to an error.")
        except Exception as e:
            logger.error(f"Error during generation of pair {serial}: {e}", exc_info=True)

if __name__ == "__main__":
    generate_bulk_pdfs(n=5)
